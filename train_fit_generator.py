...
import numpy as np
from Siraj_model import build_model
...
finish_file = 25
i = start_file
checkpoint_name = './models/model_2.4-E{epoch:03d}-vloss{val_loss:.2f}-vacc{val_acc:.2f}.h5'
...
def generate_files(start_file, finish_file):
    while True:
        for i in range(start_file, finish_file):
            try:
                file_name = 'newData/collected_data-{}_balanced.npy'.format(i)
...
#print('Memory (Before) : ' + str(mem.memory_usage()) + 'MB')
print("\nStart Fitting File ...")
model.fit_generator(generate_files(1, 25),
                    validation_data=generate_files(25, 31), validation_steps=6,
